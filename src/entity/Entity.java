...
public class Entity { // Parent class for variables that will be used in player, monster, and NPC classes
	...
	public void setAction() {
		// This exists in the sub-class Old_Man so even though empty here. It's taking the setAction() in Old_Man
	}
		...
		if(worldX + gp.tileSize > gp.player.worldX - gp.player.screenX && // Created that only tiles in boundary are drawn.
		   ...
		   worldY - gp.tileSize < gp.player.worldY + gp.player.screenY) {
			...
			if(type == 2 && hpBarOn == true) {
				...
				double oneScale = (double)gp.tileSize / maxLife; // So we know length of 1 health bar 
	...
	public void dyingAnimation(Graphics2D g2) {
		...
		int i = 5; // Interval for death animation
		
		if(dyingCounter <= i) { changeAlpha(g2, 0f); }
		if(dyingCounter > i && dyingCounter <= i * 2) { changeAlpha(g2, 1f); } // IF YOU WANT TO CREATE ADVANCED DEATHS, CAHNGE IMAGE HERE instead of Alpha
